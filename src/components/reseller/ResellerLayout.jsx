...
export default function ResellerLayout() {
  ...
  const Sidebar = (
    <aside className="w-64 bg-[#111827] text-gray-200 flex flex-col min-h-screen">
      <Link to="/" className="flex items-center gap-2 px-5 py-5 border-b border-white/10">
        <div className="bg-[#1E3A8A] h-9 w-9 rounded-lg flex items-center justify-center">
          <Store className="w-5 h-5 text-white" />
        </div>
        <span className="font-bold">Reseller Panel</span>
      </Link>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium ${
                isActive ? "bg-[#1E3A8A] text-white" : "hover:bg-white/5"
              }`
            }
          >
